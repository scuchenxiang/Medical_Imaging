...
for i = 1:180
    proj_filtered(:,i) = proj_fft(:,i).*filter;%sqrt(real(proj_fft(:,i)).^2+imag(proj_fft(:,i)).^2);%filter;
end
% figure
% subplot(1,2,1),imshow(real(proj_fft)),title('傅立叶变换')
% subplot(1,2,2),imshow(real(proj_filtered)),title('傅立叶变换+滤波')

%% 逆快速傅里叶变换并反投影
% 逆快速傅里叶变换 proj_ifft
tmp1=ifft(proj_filtered.');
    ...
    for x = 1:M
        for y = 1:M
            ...
            if t<size(R,1)/2 && t>-size(R,1)/2
                tttt=round(t+size(R,1)/2)
                tt=proj_ifft(round(t+size(R,1)/2),i)
                fbp(x,y)=fbp(x,y)+proj_ifft(round(t+size(R,1)/2),i);
            end
%             center=M/2;
%             t_temp = (x-center) * cos(rad) - (y-center) * sin(rad)+center  ;
%              %最近邻插值法
%             t = round(t_temp) ;
%             if t>0 && t<=512
...
fbp = (fbp)/180;%512x512 原图像每个像素位置的密度 *pi
